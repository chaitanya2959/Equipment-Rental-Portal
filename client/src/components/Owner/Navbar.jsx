...
function Navbar({ onToggleSidebar, isMobile }) {
  ...
  const initials = (profile?.name || "Owner")
    .split(" ")
    .map((part) => part[0])
    .slice(0, 2)
    .join("")
  ...
  return (
    <header className="owner-navbar">
      ...
      <div className="owner-navbar-actions">
        ...
        >
          <span className="owner-avatar">{avatarUrl ? <img src={avatarUrl} alt={profile?.name || "Owner"} /> : initials}</span>
          <span className="owner-profile-copy d-none d-lg-flex">
            <strong>{profile?.name || "Owner"}</strong>
            <span className="owner-role-badge">Owner</span>
          </span>
        </button>
      </div>
